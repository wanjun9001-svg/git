...
from calculation.registry import CALCULATORS, DEFAULT_CALCULATOR_KEY, MODEL_FIELD, CalculatorConfig
...
class App:
    def __init__(self) -> None:
        self.current_calc_key = DEFAULT_CALCULATOR_KEY
        self.current_calc = CALCULATORS[self.current_calc_key]
        ...
        self.header_title: ttk.Label | None = None
        self.header_subtitle: ttk.Label | None = None
        self.calc_type_var: tk.StringVar | None = None
        self.calc_type_box: ttk.Combobox | None = None
        self.input_body: tk.Frame | None = None
        self.result_body: tk.Frame | None = None
        self.steps_body: tk.Frame | None = None
        ...
        self._build_menu()
        self._build_layout()
        self._apply_calc_context(reset=True)
    ...
    def _configure_style(self) -> None:
        ...
        style.configure("Panel.Horizontal.TScrollbar", background="#d9e2ec", troughcolor="#f5f7fa", arrowcolor=TITLE_BG)
    ...
    def _build_layout(self) -> None:
        ...
        self.header_title = ttk.Label(header, text="", style="Header.TLabel")
        self.header_title.grid(row=0, column=0, sticky="w")
        self.header_subtitle = ttk.Label(header, text="", style="Muted.TLabel")
        ...
        self.calc_type_var = tk.StringVar(self.root)
        ttk.Label(toolbar, text="计算类型").grid(row=0, column=0, padx=(0, 6))
        self.calc_type_box = ttk.Combobox(
            toolbar,
            state="readonly",
            textvariable=self.calc_type_var,
            values=[cfg.title for cfg in CALCULATORS.values()],
            width=20,
        )
        self.calc_type_box.grid(row=0, column=1, padx=(0, 12))
        self.calc_type_box.bind("<<ComboboxSelected>>", self._on_calc_type_change)

        ttk.Button(toolbar, text="重新计算", command=self.calculate, style="Toolbar.TButton").grid(row=0, column=2, padx=(0, 8))
        ttk.Button(toolbar, text="导出 Word 计算书", command=self.export_word, style="Toolbar.TButton").grid(row=0, column=3, padx=(0, 8))
        ttk.Button(toolbar, text="恢复默认", command=self.reset_defaults, style="Toolbar.TButton").grid(row=0, column=4, padx=(0, 8))
        ttk.Button(toolbar, text="清空输入", command=self.clear_inputs, style="Toolbar.TButton").grid(row=0, column=5, padx=(0, 8))
        ...
        self.input_body = input_panel.body  # type: ignore[attr-defined]
        self.result_body = result_panel.body  # type: ignore[attr-defined]
        self.steps_body = steps_panel.body  # type: ignore[attr-defined]
        self._build_result_panel(self.result_body)
        self._build_steps_panel(self.steps_body)
    ...
    def _apply_calc_context(self, *, reset: bool) -> None:
        self.root.title(f"{self.current_calc.title}自动生成计算书")
        if self.header_title:
            self.header_title.configure(text=self.current_calc.title)
        if self.header_subtitle:
            self.header_subtitle.configure(text=self.current_calc.subtitle)
        if self.calc_type_var:
            self.calc_type_var.set(self.current_calc.title)
        self._rebuild_input_panel()
        self._clear_result_views()
        self._refresh_diagram()
        if reset:
            self.reset_defaults()
        else:
            self.status_var.set(f"已切换到“{self.current_calc.title}”。")

    def _on_calc_type_change(self, _event: object) -> None:
        if not self.calc_type_var:
            return
        selected_title = self.calc_type_var.get()
        for key, config in CALCULATORS.items():
            if config.title == selected_title and key != self.current_calc_key:
                self.current_calc_key = key
                self.current_calc = config
                self.last_report = None
                self._apply_calc_context(reset=True)
                return

    def _rebuild_input_panel(self) -> None:
        if not self.input_body:
            return
        for child in self.input_body.winfo_children():
            child.destroy()

        canvas = tk.Canvas(self.input_body, highlightthickness=0, bg=PANEL_BG)
        scrollbar = ttk.Scrollbar(self.input_body, orient=tk.VERTICAL, command=canvas.yview, style="Panel.Vertical.TScrollbar")
        ...
        field_map = {MODEL_FIELD.key: MODEL_FIELD}
        for field in self.current_calc.input_fields:
            field_map[field.key] = field

        self.entries = {}
        row = 0
        for group in self.current_calc.input_groups:
            section = ttk.LabelFrame(content, text=group.title, padding=10)
            section.grid(row=row, column=0, sticky="ew", pady=(0, 10))
            section.columnconfigure(1, weight=1)
            for idx, key in enumerate(group.keys):
                field = field_map[key]
                ttk.Label(section, text=field.label).grid(row=idx, column=0, sticky="w", pady=5, padx=(0, 8))
                entry = ttk.Entry(section, font=("Consolas", 10))
                entry.grid(row=idx, column=1, sticky="ew", pady=5)
                ttk.Label(section, text=field.unit, width=8, foreground=TEXT_MUTED).grid(row=idx, column=2, sticky="w", padx=(8, 0))
    ...
    def _resolve_diagram_path(self, calc: CalculatorConfig | None = None) -> Path | None:
        calc = calc or self.current_calc
        root = _project_root()
        for relative_path in calc.diagram_candidates:
            candidate = root / relative_path
            if candidate.exists():
                return candidate

        if not calc.workbook_name:
            return None

        workbook = root / "data" / calc.workbook_name
        if not workbook.exists():
            return None

        data_dir = workbook.parent
        cache_stem = data_dir / f"_diagram_cache_{calc.key}"
        try:
            with ZipFile(workbook) as archive:
                media_files = [name for name in archive.namelist() if name.startswith("xl/media/")]
                if not media_files:
                    return None
                target_name = media_files[0]
                suffix = Path(target_name).suffix.lower()
                if suffix == ".png":
                    png_path = cache_stem.with_suffix(".png")
                    if not png_path.exists() or png_path.stat().st_mtime < workbook.stat().st_mtime:
                        png_path.write_bytes(archive.read(target_name))
                    return png_path
                if suffix == ".emf":
                    emf_path = cache_stem.with_suffix(".emf")
                    png_path = cache_stem.with_suffix(".png")
                    if not png_path.exists() or png_path.stat().st_mtime < workbook.stat().st_mtime:
    ...
    def _refresh_diagram(self) -> None:
        if not self.diagram_label:
            return
        image_path = self._resolve_diagram_path()
        if not image_path or not image_path.exists():
            self.diagram_label.configure(text="未找到当前计算类型对应的简图。", image="", height=6)
    ...
    def _open_diagram_viewer(self, _event: object | None = None) -> None:
        image_path = self._resolve_diagram_path()
        if not image_path or not image_path.exists():
            messagebox.showinfo("提示", "未找到当前计算类型的计算简图。")
            return
    ...
    def _set_diagram_viewer_scale(self, action: str | tuple[int, int]) -> None:
        ...
        try:
            ...
            idx = 3
        idx = min(len(scales) - 1, idx + 1) if action == "in" else max(0, idx - 1)
    ...
    def _load_diagram_viewer_image(self, fit: bool = False) -> None:
        ...
        if fit:
            ...
            den = max(1, int(max(scale_w, scale_h, 1.0)))
            self.diagram_viewer_scale = (1, den)
    ...
    def _read_inputs(self) -> tuple[str, dict[str, float]]:
        ...
        data: dict[str, float] = {}
        for field in self.current_calc.input_fields:
            value = self.entries[field.key].get().strip()
            if value == "":
                raise ValueError(f"请输入 {field.label}")
    ...
    def calculate(self) -> None:
        try:
            ...
            report = self.current_calc.calculate_report(data)
            report["inputs"] = {"crane_model": model, **report["inputs"]}  # type: ignore[index]
            report["calc_key"] = self.current_calc.key
            report["calc_title"] = self.current_calc.title
            ...
            self.status_var.set(f"计算完成：{report['summary']}    时间：{datetime.now().strftime('%H:%M:%S')}")
    ...
    def reset_defaults(self) -> None:
        self.clear_inputs()
        for key, value in self.current_calc.default_inputs.items():
            entry = self.entries.get(key)
            if entry:
                entry.insert(0, value)
        self.status_var.set(f"已恢复“{self.current_calc.title}”默认值。")
    ...
    def _refresh_results(self, results: dict[str, object]) -> None:
        if not self.result_tree:
            return
        for item in self.result_tree.get_children():
            self.result_tree.delete(item)
        for index, field in enumerate(self.current_calc.output_fields):
            tag = "even" if index % 2 == 0 else "odd"
            value = self._format_result_value(results.get(field.key, ""))
            self.result_tree.insert("", tk.END, values=(field.label, value, field.unit), tags=(tag,))
    ...
    def _add_history(self, report: dict[str, object]) -> None:
        record = {
            ...
            "calc_key": report.get("calc_key", self.current_calc.key),
    ...
    def open_history_window_dialog(self) -> None:
        ...
        note = tk.Label(
            body,
            text="点击某条历史记录后，主界面会自动切换到对应计算类型，并显示输入参数、计算结果和计算步骤。",
            bg=PANEL_BG,
            fg=TEXT_MUTED,
            font=("Microsoft YaHei UI", 9),
            anchor="w",
        ...
        ttk.Button(action_bar, text="加载选中记录", command=self._load_selected_history, style="Toolbar.TButton").pack(side=tk.LEFT)
    ...
    def _refresh_history_list(self, *, select_last: bool = False) -> None:
        ...
        for index, item in enumerate(self.history, start=1):
            report = item["report"]
            summary = self._format_history_item(report if isinstance(report, dict) else {})
    ...
    def _format_history_item(self, report: dict[str, object]) -> str:
        calc_key = str(report.get("calc_key", self.current_calc.key))
        calc = CALCULATORS.get(calc_key, self.current_calc)
        ...
        return f"{calc.title} | {model_text}{summary}"
    ...
    def _load_report(self, report: dict[str, object]) -> None:
        calc_key = str(report.get("calc_key", self.current_calc_key))
        if calc_key in CALCULATORS and calc_key != self.current_calc_key:
            self.current_calc_key = calc_key
            self.current_calc = CALCULATORS[calc_key]
            self._apply_calc_context(reset=False)
        ...
        self.status_var.set(f"已加载历史计算结果：{report.get('summary', '')}")
    ...
    def export_word(self) -> None:
        ...
        calc_key = str(self.last_report.get("calc_key", self.current_calc.key))
        calc = CALCULATORS.get(calc_key, self.current_calc)
        initial_name = f"{calc.doc_title}.docx"
    ...
    def _write_docx(self, file_path: Path, report: dict[str, object]) -> None:
        ...
        image_path = self._resolve_diagram_path(calc)
        has_image = bool(image_path and image_path.exists())
        document_xml = self._build_document_xml(report, calc, now, has_image=has_image)
...
<cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties"
 ...
 xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
  <dc:title>{calc.doc_title}</dc:title>
...
"""
    ...
    def _build_document_xml(self, report: dict[str, object], calc: CalculatorConfig, now: str, *, has_image: bool) -> str:
        inputs = report["inputs"]  # type: ignore[assignment]
        results = report["results"]  # type: ignore[assignment]
        steps = report.get("steps", [])

        body: list[str] = []
        body.append(self._docx_paragraph(calc.doc_title, bold=True, size=40, center=True, color="1F4D78", style="Title", indent=False))
        body.append(self._docx_paragraph(f"生成时间：{now}", size=24, center=True, indent=False))
        body.append(self._docx_paragraph("", indent=False))

        crane_model = str(inputs.get("crane_model", ""))  # type: ignore[union-attr]
        body.append(self._docx_paragraph(f"1 {calc.title}", bold=True, size=28, style="Heading1", indent=False))
        body.append(self._docx_paragraph("1.1 计算目的", bold=True, size=24, style="Heading2", indent=False))
        body.append(self._docx_paragraph(self._build_purpose_text(calc, crane_model)))
        ...
        for field in calc.input_fields:
            value = inputs[field.key]  # type: ignore[index]
            param_rows.append([field.label, field.key, self._format_result_value(value), field.unit])
            ...
            for step in steps:
                if isinstance(step, dict):
                    ...
                    if title and formula:
                        body.append(self._docx_paragraph(f"{title}：{formula}"))
        if has_image:
            body.append(self._docx_paragraph("计算简图", bold=True, size=22, indent=False))
            body.append(self._docx_image_paragraph("rId1", calc))
            body.append(self._docx_paragraph(f"{calc.title}计算简图", center=True, size=19, indent=False))

        body.append(self._docx_paragraph("1.4 计算结果", bold=True, size=24, style="Heading2", indent=False))
        result_rows = [["项目", "符号", "数值", "单位"]]
        for field in calc.output_fields:
            result_rows.append([field.label, field.key, self._format_result_value(results.get(field.key, "")), field.unit])
        ...
        conclusion = str(report.get("conclusion", report.get("summary", "")))
        body.append(self._docx_paragraph(conclusion))
...
<w:body>"""
    ...
    def _build_purpose_text(self, calc: CalculatorConfig, crane_model: str) -> str:
        model_text = f"{crane_model}吨汽车吊" if crane_model else "汽车吊"
        if calc.key == "anti_overturning":
            return f"为校核{model_text}吊装作业时抗倾覆力矩是否满足要求，判断当前工况下汽车吊抗倾覆稳定性是否满足吊装要求。"
        return f"为校核{model_text}吊装作业时各支腿反力及地面最大压强，判断路基箱及地基承载条件是否满足吊装要求。"
...
"""
    ...
    def _docx_image_paragraph(self, rel_id: str, calc: CalculatorConfig) -> str:
        image_path = self._resolve_diagram_path(calc)
